...
A slice page's URL is already its slice's own stable `em export` key
(`<model-key>/slices/<sliceKey>.html`) — landing directly on a specific slice, addressable and
bookmarkable. Element-level deep links, built on em's stable `<sliceKey>/<kind>.<slug>` element
`ref`s so an agent citing a ref (via `em query`/MCP) and a stakeholder clicking a portal link
mean the same element, are MIL-173 — tracked as a fast-follow PR on top of this scaffold rather
than folded into it, so the export-ingestion/build-orchestration work here can be reviewed on
its own.
...
**Immediate fast-follow, same 0.1.0 milestone** (MIL-173, its own PR on top of this scaffold):
element-level deep links on em's stable `ref`s — see [Deep links](#deep-links-mil-173) above.
...
refuse-on-error path (an unresolvable `view ... from`).
